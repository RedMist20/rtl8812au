...
EXTRA_CFLAGS += -O1
# Do not treat warnings as errors (some kernels inject -Werror)
EXTRA_CFLAGS += -Wno-error
EXTRA_CFLAGS += -Wno-error=all
EXTRA_CFLAGS += -Wno-error=implicit-function-declaration
EXTRA_CFLAGS += -Wno-error=misleading-indentation
EXTRA_CFLAGS += -Wno-error=stringop-overread
...
# Extended warning suppression (kernel 4.4 + modern GCC compatibility)
...
EXTRA_CFLAGS += -Wno-error=format-truncation
...
### ARM64 defaults (aarch64) ###
# Kernel expects ARCH=arm64 for aarch64 systems.
ARCH ?= arm64
CROSS_COMPILE ?=
...
# Optional: extra kernel CFLAGS when building module
USER_KCFLAGS ?=
...
modules:
	$(MAKE) ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_COMPILE) -C $(KSRC) M=$(shell pwd) KCFLAGS="$(USER_KCFLAGS)" modules
strip:
